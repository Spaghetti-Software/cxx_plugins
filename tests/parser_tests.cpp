...
TEST(ParserTests, NumberTests) {
  ...
  unsigned uvar = 0;
  JSON::deserializeValue(doc_u, uvar);
  EXPECT_EQ(uvar, 4);
  EXPECT_THROW({ JSON::deserializeValue(doc_i, uvar); }, JSON::TypeMismatch);
  EXPECT_THROW({ JSON::deserializeValue(doc_f, uvar); }, JSON::TypeMismatch);

  int ivar = 0;
  JSON::deserializeValue(doc_u, ivar);
  EXPECT_EQ(ivar, 4);
  JSON::deserializeValue(doc_i, ivar);
  EXPECT_EQ(ivar, -4);
  EXPECT_THROW({ JSON::deserializeValue(doc_f, ivar); }, JSON::TypeMismatch);

  std::uint64_t u64var = 0;
  JSON::deserializeValue(doc_u, u64var);
  EXPECT_EQ(u64var, 4);
  EXPECT_THROW({ JSON::deserializeValue(doc_i, u64var); }, JSON::TypeMismatch);
  EXPECT_THROW({ JSON::deserializeValue(doc_f, u64var); }, JSON::TypeMismatch);

  std::int64_t i64var = 0;
  JSON::deserializeValue(doc_u, i64var);
  EXPECT_EQ(i64var, 4);
  JSON::deserializeValue(doc_i, i64var);
  EXPECT_EQ(i64var, -4);
  EXPECT_THROW({ JSON::deserializeValue(doc_f, u64var); }, JSON::TypeMismatch);

  float f = 0;
  JSON::deserializeValue(doc_u, f);
  EXPECT_EQ(f, 4);
  JSON::deserializeValue(doc_i, f);
  EXPECT_EQ(f, -4);
  JSON::deserializeValue(doc_f, f);
  EXPECT_EQ(f, 4.5);

  double d = 0;
  JSON::deserializeValue(doc_u, d);
  EXPECT_EQ(d, 4);
  JSON::deserializeValue(doc_i, d);
  EXPECT_EQ(d, -4);
  JSON::deserializeValue(doc_f, d);
  EXPECT_EQ(d, 4.5);
}
...
TEST(ParserTests, OptionalNumbers) {
  ...
  JSON::deserializeValue(doc_null, uvar);
  ...
  JSON::deserializeValue(doc_null, ivar);
  ...
  JSON::deserializeValue(doc_null, u64var);
  ...
  JSON::deserializeValue(doc_null, i64var);
  ...
  JSON::deserializeValue(doc_null, f);
  ...
  JSON::deserializeValue(doc_null, d);
...
TEST(ParserTests, String) {
  ...
  std::string str;
  JSON::deserializeValue(doc, str);
  EXPECT_EQ(str, "string");

  EXPECT_THROW({ JSON::deserializeValue(doc_null, str); }, JSON::TypeMismatch);
}
...
TEST(ParserTests, OptionalString) {
  ...
  JSON::deserializeValue(doc_null, str);
...
TEST(ParserTests, Vector) {
  ...
  EXPECT_THROW({ JSON::deserializeValue(doc_null, uvec); }, JSON::TypeMismatch);
  std::optional<std::vector<unsigned>> uvec_opt;
  JSON::deserializeValue(doc_null, uvec_opt);
  EXPECT_EQ(uvec_opt, std::nullopt);
  JSON::deserializeValue(doc_uint, uvec_opt);
  EXPECT_EQ(uvec_opt, expected_uint);
  EXPECT_THROW({ JSON::deserializeValue(doc_int, uvec_opt); },
               JSON::TypeMismatch);
  EXPECT_THROW({ JSON::deserializeValue(doc_double, uvec_opt); },
               JSON::TypeMismatch);

  std::vector<int> ivec;
  EXPECT_THROW({ JSON::deserializeValue(doc_null, ivec); }, JSON::TypeMismatch);
  std::optional<std::vector<int>> ivec_opt;
  JSON::deserializeValue(doc_null, ivec_opt);
  EXPECT_EQ(ivec_opt, std::nullopt);

  JSON::deserializeValue(doc_int, ivec_opt);
  EXPECT_EQ(ivec_opt.value(), expected_int);
  EXPECT_THROW({ JSON::deserializeValue(doc_double, ivec_opt); },
               JSON::TypeMismatch);

  std::vector<double> dvec;
  EXPECT_THROW({ JSON::deserializeValue(doc_null, dvec); }, JSON::TypeMismatch);
  std::optional<std::vector<double>> dvec_opt;
  JSON::deserializeValue(doc_null, dvec_opt);
  EXPECT_EQ(dvec_opt, std::nullopt);

  JSON::deserializeValue(doc_double, dvec_opt);
...
TEST(ParserTests, Map) {
  ...
  JSON::deserializeValue(doc, result);
...
TEST(ParserTests, TupleMapSizeMismatch) {
  ...
  EXPECT_THROW({ JSON::deserializeValue(doc, result); },
               JSON::ObjectSizeMismatch);
}
