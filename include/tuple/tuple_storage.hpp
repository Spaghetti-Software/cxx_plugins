...
public:
  ...
  static constexpr bool is_nothrow_copy_assign =
      (std::is_nothrow_copy_assignable_v<InnerType<Ts>> && ...);
  static constexpr bool is_nothrow_move_assign =
      (std::is_nothrow_move_assignable_v<InnerType<Ts>> && ...);
  ...
  operator=(TupleStorage const &other) noexcept(is_nothrow_copy_assign) {
    if (this != &other) {
  ...
  constexpr TupleStorage &
  operator=(TupleStorage &&other) noexcept(is_nothrow_move_assign) {
    if (this != &other) {
...
private:
  ...
  assign(std::index_sequence<indices...> /*unused*/,
         TupleStorage const &rhs) noexcept(is_nothrow_copy_assign) {
    ((impl::getInner<indices>(*this) = impl::getInner<indices>(rhs)), ...);
  }
  template <std::size_t... indices>
  constexpr void assign(std::index_sequence<indices...> /*unused*/,
                        TupleStorage &&rhs) noexcept(is_nothrow_copy_assign) {
    ((impl::getInner<indices>(*this) =
          std::move(impl::getInner<indices>(std::move(rhs)))),
     ...);
  }
